...
import (
	...
	"github.com/meedaycodes/day25-performance/internal/worker"
)
...
func (a *AuthService) Signup(ctx context.Context, cu model.CreateUserRequest) (res model.AuthResponse, err error) {
	...
	id := uuid.New().String()
	user := model.User{ID: id, Name: cu.Name, Email: cu.Email, PasswordHash: string(hashedPassword), Role: "user"}
	...
	accessToken, err := a.generateToken(user.ID, "user", time.Minute*15)
	if err != nil {
		return res, err
	}
	refreshTokenString, err := a.generateToken(user.ID, "user", time.Hour*7*24)
	if err != nil {
		return res, err
	}
...
func (a *AuthService) generateToken(userID string, role string, duration time.Duration) (string, error) {
...
func (a *AuthService) Login(ctx context.Context, lg model.LoginRequest) (auth model.AuthResponse, err error) {
	...
	accessToken, err := a.generateToken(user.ID, user.Role, time.Minute*15)
	if err != nil {
		return auth, err
	}
	refreshTokenString, err := a.generateToken(user.ID, user.Role, time.Hour*7*24)
	if err != nil {
		return auth, err
	}
