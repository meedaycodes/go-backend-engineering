...
// ReadWriteUserRepository implements UserRepository using PostgreSQL as the
...
type ReadWriteUserRepository struct {
	write *pgxpool.Pool // primary — all writes go here
	read  *pgxpool.Pool // replica — all reads go here
}

// NewPostgresUserRepository creates a new NewPostgresUserRepository with the given
...
func NewReadWriteUserRepository(write, read *pgxpool.Pool) *ReadWriteUserRepository {
	return &ReadWriteUserRepository{write: write, read: read}
}
...
func (r *ReadWriteUserRepository) Save(ctx context.Context, user model.User) error {

	sqlStatement := "INSERT INTO users (id, name, email, password_hash) VALUES ($1,$2,$3,$4)"

	cmdTag, err := r.write.Exec(ctx, sqlStatement, user.ID, user.Name, user.Email, user.PasswordHash)
...
func (r *ReadWriteUserRepository) FindByID(ctx context.Context, id string) (model.User, error) {

	var user model.User

	sqlStatement := "SELECT id, name, email,password_hash, role  FROM users WHERE id = $1"

	row := r.read.QueryRow(ctx, sqlStatement, id)
...
func (r *ReadWriteUserRepository) FindAll(ctx context.Context, limit, offset int) ([]model.User, error) {

	var users []model.User

	sqlStatement := "SELECT id, name, email, password_hash, role FROM users LIMIT $1 OFFSET $2"
	rows, err := r.read.Query(ctx, sqlStatement, limit, offset)
...
func (r *ReadWriteUserRepository) Update(ctx context.Context, user model.User) error {

	sqlStatement := "UPDATE users SET name = $1 , email = $2 WHERE id = $3"
	cmdTag, err := r.write.Exec(ctx, sqlStatement, user.Name, user.Email, user.ID)
...
func (r *ReadWriteUserRepository) Delete(ctx context.Context, id string) error {

	sqlStatement := "DELETE FROM users WHERE id = $1"

	cmdTag, err := r.write.Exec(ctx, sqlStatement, id)
...
// pgx returns pgx.ErrNoRows which we translate to ErrUserNotFound to keep
// error handling consistent across repository implementations.
func (r *ReadWriteUserRepository) FindByEmail(ctx context.Context, email string) (model.User, error) {

	var user model.User

	sqlStatement := "SELECT id, name, email,password_hash, role  FROM users WHERE email = $1"

	row := r.read.QueryRow(ctx, sqlStatement, email)
	err := row.Scan(&user.ID, &user.Name, &user.Email, &user.PasswordHash, &user.Role)
