...
func (p *PostgresUserRepository) Save(ctx context.Context, user model.User) error {
	...
	sqlStatement := "INSERT INTO users (id, name, email, password_hash, role) VALUES ($1,$2,$3,$4, $5)"

	cmdTag, err := p.pool.Exec(ctx, sqlStatement, user.ID, user.Name, user.Email, user.PasswordHash, user.Role)
