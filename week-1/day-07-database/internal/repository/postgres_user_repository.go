...
import (
	...
	"github.com/meedaycodes/day-07-database/internal/model"
)
...
func (p *PostgresUserRepository) Save(ctx context.Context, user model.User) error {
	...
	sqlStatement := "INSERT INTO users (id, name, email) VALUES ($1,$2,$3)"

	cmdTag, err := p.pool.Exec(ctx, sqlStatement, user.ID, user.Name, user.Email)
...
func (p *PostgresUserRepository) FindAll(ctx context.Context) ([]model.User, error) {

	var users []model.User

	sqlStatement := "SELECT id, name, email FROM users"
	rows, err := p.pool.Query(ctx, sqlStatement)
