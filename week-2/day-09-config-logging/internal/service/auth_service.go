...
import (
	...
	"github.com/meedaycodes/day09-config-logging/internal/model"
...
// It depends on UserRepository (interface) for data access and holds the
// JWT secret for signing tokens. Both are injected via the constructor.
...
// hashes the password with bcrypt, saves the user, and returns access + refresh
// tokens. Bcrypt uses a random salt so the same password produces different
// hashes each time — verified with CompareHashAndPassword, not string comparison.
