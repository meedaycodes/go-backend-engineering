...
func (u *UserHandler) GetAllUsers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Wrong value for limit", http.StatusBadRequest)
		return
	}
	if intLimit > 100 {
		intLimit = 10
	...
	if err != nil {
		http.Error(w, "Wrong value for offset", http.StatusBadRequest)
		return
	}

	users, err := u.service.GetAllUsers(ctx, intLimit, intOffset)
